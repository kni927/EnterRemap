...
- 🟢 稼働中(Running)
- 🟡 一時停止中(Paused。メニューから手動で切り替え。プロセスは終了しない)
- 🔴 イベントタップが無効化され、既存の自動再有効化ロジックでも
  復帰できなかった状態(Tap Recovery Failed。要再起動)

アイコンをクリックすると状態表示・「Pause/Resume」・「Quit」の
メニューが出る(UIは英語表記。日本語はREADMEのみ)。一時停止中は
Enter/Cmd+Enterともに完全に素通しになる
